...
本次更新新增侧边栏 `生成参数` 接管，建议需要从 Bridge 面板直接控制出图尺寸、Steps 或 Seed 的用户更新。

- 可分别开关接管出图宽高、Steps、Seed 和 Seed 模式。
- 有上游连接时读取/写入上游参数，没有上游时使用节点内置参数。
- 修复宽高没有映射到真实出图尺寸、Seed 与 KSampler 实际值不一致的问题。
- 已通过真实 ComfyUI 生图验证：上游/内置宽高和上游/内置 Seed 均可用。
