...
本次更新继续打磨节点布局和旧工作流兼容性，建议所有 v0.4.x 用户更新。

- 修复点击 `恢复尺寸` 后 LoRA 区域来回抖动的问题。
- 修复拖动提示词标签区和 LoRA 区后，底部边框偶发大片空白的问题。
- 优化 LoRA 卡片区高度，重启后不再异常变长。
- 修复旧工作流中右侧开关被 Bridge 节点遮挡、按钮点不动的问题。
