...
本次更新重点修复旧示范工作流、生成按钮和节点拖拽相关的稳定性问题，建议所有 v0.4.x 用户更新。

- 旧版示范工作流重新可用，打开后不再因为字段顺序变化而报错。
- 点击 Bridge 内的 `Generate` 后，节点界面不再被压成窄列或出现大片空白。
- 鼠标在节点内滚轮时只滚动节点内容，不再误缩放 ComfyUI 画布。
- 生成后拖拽调整节点尺寸不再导致页面无响应。
- 更新后请重启 ComfyUI，并强制刷新浏览器页面。

## v0.4.x 更新概要

v0.4.x 把 Bridge 从单纯提示词节点扩展成一个可日常使用的 WebUI 式工作台：内置提示词/反向词编辑、LoRA 浏览与真实加载、模型切换、布局预设、区域控制、图生图和常用高级模块入口。

这一路更新主要集中在三件事：让旧工作流继续兼容、让节点布局和拖拽更稳定、让 LoRA/提示词编辑更接近 WebUI 的操作手感。
