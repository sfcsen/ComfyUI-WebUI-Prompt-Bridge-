...
这一版是一个轻量布局更新，给提示词面板增加了两个新的工作模式：`正向撰写优先` 和 `隐藏 LoRA 最小化`。

`正向撰写优先` 会放大正向 Prompt 和正向标签区，并折叠反向提示词细节，适合集中写正向提示词。`隐藏 LoRA 最小化` 会折叠 LoRA 区并缩小节点，适合只写提示词、不想让 LoRA 卡片占用空间的场景。

面板右侧现在有 `默认`、`紧凑`、`宽松`、`正向`、`极简` 五个快捷布局按钮，不用打开设置页也可以快速切换。布局设置会保存到本地配置里，重启 ComfyUI 后仍会保留。
