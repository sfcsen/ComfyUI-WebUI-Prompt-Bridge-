...
- **收藏更可靠**：收藏列表不再只显示前 260 条，并可直接在列表中取消收藏。
- **迁移目录可重新识别**：自动检测支持 `盘符:/AI/Tools/sd-webui-aki-*`，也可直接粘贴 `models/Lora` 目录。

完整细节见 [CHANGELOG.md](CHANGELOG.md) 和 [v0.4.26 发布说明](docs/release-notes-v0.4.26.md)。更新后请重启 ComfyUI，并按 `Ctrl+F5` 强制刷新浏览器页面。
