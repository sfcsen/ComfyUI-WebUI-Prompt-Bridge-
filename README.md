...
- **主界面更清楚**：正向词、反向词改为页签切换，LoRA 搜索、分页和排序重新排布，窄节点也不再堆满按钮。
- **提示词市场更好找**：明确这是可导入主节点与提示词小节点的词库来源，并新增画风、镜头、光影、材质和质量增强等入口。
- **高级模块会先检查资产**：ADetailer、ControlNet、SAM/Inpaint 和放大修复会显示可用、部分可用或缺少资产，不再把“装了预处理器”误当成模型已经齐全。
- **一键构建可安全还原**：构建后会显示新增节点、自动连线和待接输入；可以直接还原本次构建。
- **手动接入不再伪装完成**：SAM 只创建 Mask/Inpaint 接口；Ultimate/Tiled 只保存参数，明确提示用户连接对应的第三方节点。
- **模型扫描更可靠**：忽略下载中、损坏和占位文件，路径统一并去重，减少资产数量虚高和错误默认模型。

完整细节见 [CHANGELOG.md](CHANGELOG.md) 和 [v0.4.25 发布说明](docs/release-notes-v0.4.25.md)。更新后请重启 ComfyUI，并强制刷新浏览器页面。
