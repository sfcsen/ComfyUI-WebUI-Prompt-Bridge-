...
这一版主要修复提示词面板里几个会直接影响使用体验的问题。反向提示词里的 `收藏列表` 和 `扩展模型` 现在可以正常打开，不会点过去又跳回反向词；`扩展模型` 里也加入了 Embedding，常用 embedding 可以像普通标签一样直接点进提示词。

联网翻译也做了修复。开启联网翻译后，输入中文再点 `英` 会正常翻译成英文，不再出现看起来点了但没有反应的情况。没有 WebUI 或 WebUI 路径配置不对时，设置页也会更稳定地显示当前状态，不会因为路径检测失败就变成安装失败。

另外，标签区的显示更舒服了：Prompt All in One 面板不再挤成一团，自动补全弹窗更紧凑；在设置里调大字体或换字体时，标签卡片会自动长高，中文和英文不会互相盖住。
